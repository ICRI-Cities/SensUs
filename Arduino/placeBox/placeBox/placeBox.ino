...
int yesNoButtons2[] = {27,35};
int yesNoButtons3[] = {33,25};
...
int yesNoLEDs2[] = {49,41};
int yesNoLEDs3[] = {43,24};
...
String transportAnswers[] = {"0","1","2","3","4","more than 4","pnts"};
String findInfoAnswers[] = {"Friends and family","local resources","online social network","other online resources","other","pnts"};
...
void loop() {
  ...
  if(!success){  //NO CARD INSERTED YET
    ...
    if(startOn){  //toggle start LEDs off
      ...
      if(val <= 70){
        results[15] = transportAnswers[6];
      }else if(val > 70 && val <= 221){
       results[15] = transportAnswers[5];
      }else if(val > 221 && val <= 401){
       results[15] = transportAnswers[4];
      }else if(val > 401 && val <= 607){
       results[15] = transportAnswers[3];
      }else if(val > 607 && val <= 751){
       results[15] = transportAnswers[2];
      }else if(val > 751 && val <= 883){
       results[15] = transportAnswers[1];
      }else{ //val > 883 
        results[15] = transportAnswers[0];
      }
...
void turnOnSliderLED(int sliderVal){
  int ledIndex;
  if(sliderVal <= 82){
    results[2] = findInfoAnswers[5];
    ledIndex = 0;
  }else if(sliderVal > 82 && sliderVal <= 290){
    results[2] = findInfoAnswers[4];
    ledIndex = 1;
  }else if(sliderVal > 290 && sliderVal <= 503){
    results[2] = findInfoAnswers[3];
    ledIndex = 2;
  }else if(sliderVal > 503 && sliderVal <= 725){
    results[2] = findInfoAnswers[2];
    ledIndex = 3;
  }else if(sliderVal > 725 && sliderVal <= 956){
    results[2] = findInfoAnswers[1];
    ledIndex = 4;
  }else{  //sliderVal > 956
    results[2] = findInfoAnswers[0];
    ledIndex = 5;
  }
