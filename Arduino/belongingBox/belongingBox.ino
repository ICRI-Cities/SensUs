...
//button pins
int yesNoButtons1[] = {23,31};
int yesNoButtons2[] = {25,33};
int yesNoButtons3[] = {27,35};
int yesNoButtons4[] = {29,37};
...
//led pins
int yesNoLEDs1[] = {53,45};
int yesNoLEDs2[] = {24,43};
int yesNoLEDs3[] = {49,41};
int yesNoLEDs4[] = {47,39};
int startLED = 48;
int finishLED = 22;
...
int toggle1LEDs[] = {20,19,18};
int toggle2LEDs[] = {17,16,15};
int toggle3LEDs[] = {14,13,12};
...
String namesAnswers[] = {"0","1-2","3-4","5-6","7+","pnts"};
String liveInAnswers[] = {"housing shelter","social housing","family","rental","single home owner","multiple home owner","other","pnts"};
...
void loop() {
  ...
  if(!success){  //NO CARD INSERTED YET
    ...
    if(startOn){  //toggle start LEDs off
      ...
      if(val <= 160){
        results[15] = namesAnswers[5];
      }else if(val > 160 && val <= 338){
        results[15] = namesAnswers[4];
      }else if(val > 338 && val <= 528){
        results[15] = namesAnswers[3];
      }else if(val > 528 && val <= 729){
        results[15] = namesAnswers[2];
      }else if(val > 729 && val <= 903){
        results[15] = namesAnswers[1];
      }else{  // >903
        results[15] = namesAnswers[0];
      }
...
void turnOnSliderLED(int sliderVal){
  int ledIndex;
  if(sliderVal <= 50){
    results[2] = liveInAnswers[7];
    ledIndex = 7;
  }else if(sliderVal > 50 && sliderVal <= 207){
    results[2] = liveInAnswers[6];
    ledIndex = 6;
  }else if(sliderVal > 207 && sliderVal <= 361){
    results[2] = liveInAnswers[5];
    ledIndex = 5;
  }else if(sliderVal > 361 && sliderVal <= 514){
    results[2] = liveInAnswers[4];
    ledIndex = 4;
  }else if(sliderVal > 514 && sliderVal <= 668){
    results[2] = liveInAnswers[3];
    ledIndex = 3;
  }else if(sliderVal > 668 && sliderVal <= 822){
    results[2] = liveInAnswers[2];
    ledIndex = 2;
  }else if(sliderVal > 822 && sliderVal <= 974){
    results[2] = liveInAnswers[1];
    ledIndex = 1;
  }else{  //sliderVal > 974
    results[2] = liveInAnswers[0];
    ledIndex = 0;
  }
